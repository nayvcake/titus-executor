...
func configureTransitionAssignment(ctx context.Context, branchLink netlink.Link, transitionNamespaceDir string, pid1dirfd int, assignment *vpcapi.AssignIPResponseV3) error {
	...
	err := os.Mkdir(transitionNamespaceDir, 0755)
	if err != nil && !os.IsExist(err) {
		return fmt.Errorf("Unable to mkdir %q: %w", transitionNamespaceDir, err)
	}

	transitionNamespaceDirFile, err := os.Open(transitionNamespaceDir)
	if err != nil {
		return fmt.Errorf("Could not open transition namespace directory %q: %w", transitionNamespaceDir, err)
	}

	defer transitionNamespaceDirFile.Close()
	...
	for ctx.Err() != nil {
		...
		if err != unix.EWOULDBLOCK {
			return fmt.Errorf("Error locking %q: %w", transitionNamespaceDirFile.Name(), ctx.Err())
		}

		select {
		case <-ctx.Done():
			return fmt.Errorf("Timed out: Could not lock %q: %w", transitionNamespaceDirFile.Name(), ctx.Err())
...
func getTransitionNamespace(ctx context.Context, branchLink netlink.Link, transitionNamespaceDirFile *os.File, assignment *vpcapi.AssignIPResponseV3_TransitionAssignment) (*os.File, error) {
	...
	_, _, errno = syscall.Syscall6(unix.SYS_MOVE_MOUNT, srctree, uintptr(unsafe.Pointer(emptyPath)), transitionNamespaceDirFile.Fd(), uintptr(unsafe.Pointer(assignmentIDPath)), MOVE_MOUNT_F_EMPTY_PATH, 0)
	if errno != 0 {
...
func crossMount(ctx context.Context, src *os.File, pid1dirfd int) error {
	...
	err = unix.Mkdirat(root, "run", 0755)
	if err != nil && err != unix.EEXIST {
		return fmt.Errorf("Could not create run dir: %w", err)
	}

	err = unix.Mkdirat(root, "run/netns", 0755)
	if err != nil && err != unix.EEXIST {
		return fmt.Errorf("Could not create run/netns dir: %w", err)
	}

	// Make sure that this somehow didn't exist before.
	fd, err := unix.Openat(root, "run/netns/transition", unix.O_CLOEXEC|unix.O_CREAT|unix.O_EXCL, 0755)
	if err != nil {
		return fmt.Errorf("Could not create transition mount point: %w", err)
	}
	transitionMountPointFD := os.NewFile(uintptr(fd), "transition")
	defer transitionMountPointFD.Close()

	fd, err = unix.Openat(pid1dirfd, "ns/mnt", unix.O_CLOEXEC, 0)
	if err != nil {
		return fmt.Errorf("Could not open container mount ns: %w", err)
	}
	containerMountNamespaceFD := os.NewFile(uintptr(fd), "mount")
	defer containerMountNamespaceFD.Close()

	// TODO: REWRITE
	cmd := exec.CommandContext(ctx, "/usr/bin/nsenter", "--mount=/proc/self/fd/3", "/bin/mount", "--bind", "/proc/self/fd/4", "/proc/self/fd/5")
	cmd.ExtraFiles = []*os.File{
		containerMountNamespaceFD,
		src,
		transitionMountPointFD,
	}
	return cmd.Run()
}
