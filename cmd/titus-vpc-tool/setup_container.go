...
func setupContainercommand(ctx context.Context, v *pkgviper.Viper, iipGetter instanceIdentityProviderGetter) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			transitionNamespaceDir := v.GetString("transition-namespace-dir")
	...
	cmd.Flags().String("transition-namespace-dir", "/run/transition", "Directory to mount transition namespaces into")
	return cmd
}
